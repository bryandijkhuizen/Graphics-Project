  ...
  */
  public List findPath(AStarNode startNode, AStarNode goalNode) {
    ...
    while (!openList.isEmpty()) {
        //het beginpunt wordt verwijderd omdat het niet bij de route hoort. 
      AStarNode node = (AStarNode)openList.removeFirst();
      //als er nog maar 1 node overblijft is dat de route. 
      if (node == goalNode) {
        // construct the path from start to goal
        return constructPath(goalNode);
      }
      //de naastgelegen nodes worden opgevraagd
      List neighbors = node.getNeighbour(node);
      for (int i=0; i<neighbors.size(); i++) {
        AStarNode neighborNode = (AStarNode)neighbors.get(i);
        boolean isOpen = openList.contains(neighborNode);
        boolean isClosed = closedList.contains(neighborNode);
        float costFromStart = node.costFromStart + node.getCost(neighborNode, goalNode);
