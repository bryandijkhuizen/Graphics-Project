...
public String DijkstraShortestPath(NodeWeighted start, NodeWeighted end) {
    ...
    List<NodeWeighted> NodesSorted = new ArrayList<>();
    for(NodeWeighted n : nodes) {
        NodesSorted.add(n);
    }
    List<NodeWeighted> sortedList = NodesSorted.stream().sorted().collect(Collectors.toList());   
    for (NodeWeighted node : sortedList) {
