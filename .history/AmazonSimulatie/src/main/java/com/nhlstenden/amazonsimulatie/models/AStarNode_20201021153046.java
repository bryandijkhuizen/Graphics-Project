...
public abstract class AStarNode implements Comparable {
  ...
  */
  public abstract float getEstimatedCost(AStarNode node, AStarNode goalnode){
    //h = abs (current_cell.x – goal.x) + 
    //    abs (current_cell.y – goal.y) 
    float est = (node[x] - goalnode[x])
    return Math.abs(node.getGridX() - goalnode.getGridX()) + Math.abs(node.getGridY() - goalnode.getGridY());
  }
